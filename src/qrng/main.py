...
def _append_ctrl(qc, gate, ctrls, target, state):
    if ctrls:
        qc.append(
            gate.control(len(ctrls), ctrl_state=int(state[::-1], 2)), ctrls + [target]
        )
    else:
...
def _prep(qc, qs, N, ctrls=None, state=""):
    ...
    if N == full:
        for q in qs:
            _append_ctrl(qc, HGate(), ctrls, q, state)
        return
    ...
    if left:
        _prep(qc, rest, left, ctrls + [msb], state + "0")
    if right:
        _prep(qc, rest, right, ctrls + [msb], state + "1")
